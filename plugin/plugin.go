...
func (p *plugin) generateStringValidator(variableName string, ccTypeName string, fieldName string, fv *validator.FieldValidator) {
	...
	if fv.Uuid != nil {
		p.P(`if `, variableName, ` != "" && !`, p.regexPkg.Use(), `.MustCompile("[a-f0-9]{8}-[a-f0-9]{4}-4[a-f0-9]{3}-[8|9|aA|bB][a-f0-9]{3}-[a-f0-9]{12}").MatchString(`, variableName, `) {`)
		p.In()
	...
	if fv.Email != nil {
		p.P("if ", variableName, " != \"\" && !", p.regexPkg.Use(), ".MustCompile(`.+@.+\\..+`).MatchString(", variableName, ") {")
		p.In()
